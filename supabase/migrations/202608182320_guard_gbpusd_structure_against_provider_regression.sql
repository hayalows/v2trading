...
begin
  ...
  -- GBPUSD continuity guard: do not let a temporarily stale provider response
  -- regress structure behind a candle already validated by the previous state.
  if new.symbol='GBPUSD' and tg_op='UPDATE' and lag_bars > 0 then
    ...
    select max(ts) into persisted_last
    from public.market_bars
    where symbol='GBPUSD'
      and timeframe='15m'
      and coalesce(is_proxy,false)=false
      and source ilike 'Yahoo Finance public chart%';

    if incoming_last is not null
       and old_last is not null
       and persisted_last is not null
       and persisted_last > incoming_last
       and old_last = persisted_last
       and coalesce(old.formation_code,'') <> 'DATA_DEGRADED' then
      ...
      if old.details->'trends'->'m15' is not null then
        merged_details := jsonb_set(merged_details,'{trends,m15}',old.details->'trends'->'m15',true);
      end if;
      if old.details->'formation' is not null then
        merged_details := jsonb_set(merged_details,'{formation}',old.details->'formation',true);
      end if;
      if old.details->'structure_reference_price' is not null then
        merged_details := jsonb_set(merged_details,'{structure_reference_price}',old.details->'structure_reference_price',true);
      end if;
      if old.details->'diagnostics' is not null then
        merged_details := jsonb_set(merged_details,'{diagnostics}',old.details->'diagnostics',true);
      end if;
      merged_details := merged_details || jsonb_build_object(
        'providerRegressionGuard',true,
        'regressedProviderLastM15',incoming_last,
        'preservedValidatedLastM15',persisted_last
      );
      new.details := merged_details;

      dh := coalesce(old.data_health,'{}'::jsonb) || jsonb_build_object(
        'lastM15Bar',persisted_last,
        'expectedLastCompletedM15',expected_last,
        'structureLagBars',persisted_lag,
        ...
        'structureUsable',persisted_lag=0,
        'blockedReason',case when persisted_lag=0 then null else format('Structure withheld — feed %s bar(s) behind',persisted_lag) end,
        'circuitBreaker',true,
        'providerRegressionGuard',true,
        'regressedProviderLastM15',incoming_last,
        'preservedValidatedLastM15',persisted_last
      ...
      gap_count := coalesce(nullif(dh->>'gapCount96','')::integer, 0);
