...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const tradeMetrics=trades.filter((t:any)=>t.entry_at).map((t:any)=>{const mark=marks.get(t.symbol)??null,riskUsd=frozenRisk.get(t.trade_key)??null,pips=pipMetrics(t,mark);return{tradeKey:t.trade_key,symbol:t.symbol,direction:t.direction,status:t.status,entryAt:t.entry_at,exitAt:t.exit_at,riskUsd,realizedPnlUsd:n(t.gross_r)!=null&&riskUsd!=null?riskUsd*Number(t.gross_r):null,grossR:n(t.gross_r),...pips}});
