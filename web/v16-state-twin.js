(()=>{
  ...
  function analogCopy(a){
    if(!a)return {value:'Building',sub:'Historical analog layer is loading.'};
    ...
    return {value:'Withheld',sub:`${a.candidateEpisodes??0} de-correlated live episodes found. Current-state probability stays hidden until the multi-year gate passes.`};
  }
  function stabilityCopy(x){
    if(!x)return 'Loading regime-change diagnostics.';
    const cp=x.recentChangePeak==null?'—':pct(x.recentChangePeak,1),run=Number.isFinite(Number(x.expectedRunBars))?`${n(x.expectedRunBars)} bars`:'—';
    return `Bayesian change peak ${cp} · estimated current run ${run} · volatility ratio ${n(x.volatilityRatio,2)}×.`;
  }
  function renderTwin(){
    ...
    card.innerHTML=`<div class="stateTwinHead"><div><div class="stateTwinKicker"><span class="material-symbols-rounded">neurology</span>StateTwin intelligence</div><h3>${safe(x.mode?.label||'Market state loading')}</h3><p>Structural state, regime-break pressure and cross-pair context. Research intelligence, not a buy/sell instruction.</p></div><span class="stateTwinPill ${tone(x.stability?.label)}">${safe(x.stability?.label||'loading')}</span></div>
      <div class="stateTwinGrid">
        ...
        <article><span>Historical analog</span><b>${safe(a.value)}</b><small>${safe(a.sub)}</small></article>
  ...
  function ensureResearchCard(){
    ...
    card.innerHTML='<div class="cardLabel"><span class="material-symbols-rounded">neurology</span>V2 StateTwin v1.6</div><h3>Predict structural transitions, not the next candle.</h3><p>The live layer combines the existing multi-timeframe V2 state with Bayesian online changepoint detection, de-correlated historical state similarity and EURUSD/GBPUSD co-movement. If the independent sample is weak, it abstains instead of printing a probability.</p><div class="v15ResearchNote"><strong>Foundation-model policy:</strong> Chronos-2/Chronos-Bolt, TimesFM, MOMENT and patch/representation models remain challenger models. They only enter the live stack if walk-forward testing improves calibration beyond the simpler structural baseline. The earlier broker-execution failure still blocks live-money claims.</div>';
