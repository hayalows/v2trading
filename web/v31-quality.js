(()=>{
  ...
  function cardHTML(m,compact=false){
    ...
    const risk=floorText(c),friction=frictionText(c);
    return `<div class="v31QualityHead"><div><span>Setup quality</span><strong>${esc(q.label)}</strong></div><span class="v31QualityBadge ${esc(q.tone)}">${esc(m.symbol)}</span></div><p>${esc(q.reason)}</p>${risk?`<p class="v31QualityNote">${esc(risk)}</p>`:''}${friction?`<p class="v31QualityWarn">${esc(friction)}</p>`:''}${!compact?`<div class="v31Next"><b>What now</b><span>${esc(q.next)}</span></div>${details(m)}`:''}`;
  }
  function renderHome(){const root=$('#focusBoard');if(!root)return;const m=model(),sig=JSON.stringify([m.symbol,m.current?.tradeKey,m.current?.quality?.code,m.current?.risk?.pips,m.stats?.wins,m.stats?.losses,m.stats?.timeouts]);let card=$('#v31SetupQuality',root);if(!card){card=document.createElement('article');card.id='v31SetupQuality';card.className='v31QualityCard';root.querySelector('.focusHero')?.insertAdjacentElement('afterend',card)}if(card&&card.dataset.sig!==sig){card.dataset.sig=sig;card.innerHTML=cardHTML(m)}}
  function renderTrade(){const host=$('#paperCurrent');if(!host)return;const m=model(),c=m.current;if(!c||!c.tradeKey){$('#v31TradeQuality',host)?.remove();return}const sig=JSON.stringify([m.symbol,c.tradeKey,c.quality?.code,c.risk?.pips,c.friction?.status]);let card=$('#v31TradeQuality',host);if(!card){card=document.createElement('div');card.id='v31TradeQuality';card.className='v31TradeQuality';host.prepend(card)}if(card.dataset.sig!==sig){card.dataset.sig=sig;card.innerHTML=cardHTML(m,true)}}
