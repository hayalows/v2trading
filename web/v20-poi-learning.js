(()=>{
  ...
  function depthLearningCard(){
    const d=v20Data?.depthLearning;if(!d)return '<article class="card"><h3>Depth learning unavailable</h3></article>';
    ...
    return `<article class="card v20DepthLearning"><div class="cardLabel"><span class="material-symbols-rounded">experiment</span>POI depth learning</div><h3>50% stays baseline. Alternatives learn in shadow.</h3><p>The M5 historical scan found 40% descriptively stronger in the pooled sample, but the chronological test was worse than midpoint (${num(h.walkforwardDeltaR,3)}R; 95% interval ${num(h.walkforward95?.[0],3)} to ${num(h.walkforward95?.[1],3)}). V2 therefore did not change the production paper-entry rule.</p><div class="paperMeta">${chip(`Prospective shadow rows ${pros}`)}${chip(`Backfilled rows ${back}`)}${chip('No automatic promotion')}</div><div class="v20DepthGrid">${rows}</div><div class="paperNotice"><strong>Prospective gate</strong>Backfilled observations never count toward promotion. A depth’s performance remains hidden until at least 30 scored prospective observations, and no depth can alter Focus automatically.</div></article>`;
  }
  function rewriteVisiblePaperPlan(symbol){
    ...
    if(overview&&old&&!active){
      overview.innerHTML=`<article class="card"><div class="cardLabel"><span class="material-symbols-rounded">smart_toy</span>Automatic paper trade</div><h3>No current baseline plan</h3><p>The older ${esc(symbol)} plan is still being tracked for research, but it has been removed from Focus because ${esc((reason[old.focus_suppression_reason]||'its lifecycle evidence is stale').toLowerCase())}.</p><div class="paperMeta">${chip('50% baseline unchanged')}${chip('Research watch retained')}</div></article>`;
    }
  ...
  function render(){
    if(!v20Data)return;const symbol=currentSymbol();
    ...
    const research=document.getElementById('poiDepthLearning');if(research)research.innerHTML=depthLearningCard().replace(/^<article class="card v20DepthLearning">|<\/article>$/g,'');
  ...
  refresh();setInterval(refresh,60_000);setInterval(render,5_000);
})();
