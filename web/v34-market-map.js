(()=>{
  ...
  function ensure(){
    const root=$('#focusBoard');if(!root)return null;
    let card=$('#v34MarketMap',root);
    if(!card){card=document.createElement('article');card.id='v34MarketMap';card.className='v34MapCard';const grid=root.querySelector('.focusGrid'),note=root.querySelector('.v15ResearchNote');(note||grid||root.lastElementChild)?.insertAdjacentElement('afterend',card)}
    return card;
  }
  ...
  function render(){
    ...
    if(!mi){card.innerHTML='<div class="v34MapHead"><div><span>Market map</span><strong>Building higher-timeframe context</strong></div><small>Context only</small></div>';return}
  ...
  const root=$('#focusBoard');if(root)new MutationObserver(()=>requestAnimationFrame(render)).observe(root,{childList:true});
  ensure();load();setInterval(load,30000);
})();