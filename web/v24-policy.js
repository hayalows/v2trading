(()=>{
  ...
  async function load(){if(loading)return;loading=true;try{const [a,b]=await Promise.all([fetch(LAB,{cache:'no-store'}),fetch(PAPER,{cache:'no-store'})]);if(!a.ok||!b.ok)throw new Error(`policy ${a.status}/${b.status}`);data={lab:await a.json(),paper:await b.json()};render()}catch(e){console.warn('V2.4 policy',e);const r=ensure();if(r&&!data)r.innerHTML='<div class="v24Error">Account research is temporarily unavailable. The underlying paper engine is unchanged.</div>'}finally{loading=false}}
