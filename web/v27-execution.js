(()=>{
  ...
  function renderAccountEvidence(data){
    ...
    const confirmed=a.executionConfirmedEntries??a.tickConfirmedEntries??0,audited=a.executionAudited??0,html=`<strong>Execution audit</strong>${confirmed} of ${audited} audited historical entries currently have independent BID/ASK confirmation. This audit is shadow-only and does not change the balance.`;
  ...
  function renderTradeEvidence(data){
    ...
    document.querySelectorAll('.tradeRow[data-trade-key]').forEach(row=>{const m=by.get(row.dataset.tradeKey);if(!m)return;let box=row.querySelector('[data-v27-exec]');if(!box){box=document.createElement('div');box.className='paperNotice';box.dataset.v27Exec='1';row.appendChild(box)}const q=data.marketQuality?.[m.symbol],a=m.executionAudit,html=`<strong>Execution evidence · ${escx(a?.details?.confidence==='tick'?'TICK':a?.details?.confidence==='1m'?'1-MINUTE':'UNVERIFIED')}</strong>${escx(executionCopy(a))}${q?` <span style="opacity:.8">Recent ${escx(m.symbol)} context: ${escx(qualityCopy(q))}</span>`:''}`;if(box.innerHTML!==html)box.innerHTML=html});
  }
  function surfacesIntact(){
    if(!document.getElementById('v27ExecutionQuality')||!document.querySelector('#paperAccount [data-v27-account]'))return false;
    const rows=[...document.querySelectorAll('.tradeRow[data-trade-key]')];if(!rows.length)return true;
    return rows.every(r=>r.querySelector('[data-v27-exec]'));
  }
  let sig='';
  function render(){const data=window.v2PaperAccount;if(!data?.account)return;const next=`${data.generatedAt||''}:${data.account.executionAudited||0}:${data.marketQuality?.EURUSD?.asOf||''}:${data.marketQuality?.GBPUSD?.asOf||''}`;if(next===sig&&surfacesIntact())return;sig=next;renderDataCard(data);renderAccountEvidence(data);renderTradeEvidence(data)}
