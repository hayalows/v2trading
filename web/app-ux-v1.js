...
function setupHome(){
  ...
  function renderFocus(top){
    ...
    if(levels.Entry)levelBits.push(`<div><span>Entry</span><b>${esc(levels.Entry)}</b></div>`);if(levels.SL)levelBits.push(`<div><span>SL</span><b>${esc(levels.SL)}</b></div>`);if(levels.TP)levelBits.push(`<div><span>TP</span><b>${esc(levels.TP)}</b></div>`);if(!levelBits.length&&levels.Midpoint)levelBits.push(`<div><span>Midpoint</span><b>${esc(levels.Midpoint)}</b></div>`);
    focus.innerHTML=`<div class="homeFocusTop"><div><span class="homeFocusEyebrow">${label}</span><div class="homeFocusTitle"><strong>${esc(top.sym)}</strong><span>${esc(top.price)}</span></div></div><span class="homeFocusStage">Stage ${top.stage}/8</span></div><div class="homeFocusStatus">${esc(status)}</div>${fastMarkup(top)}${levelBits.length?`<div class="homeFocusLevels">${levelBits.join('')}</div>`:''}<div class="homeFocusNext"><span>Next</span><b>${top.next?esc(top.next):'Monitor the next completed structural event.'}</b></div>`;focus.hidden=false;
  }
  function renderLatest(){const card=$('#tradeList .tradeCard');if(!card){latest.hidden=true;latest.innerHTML='';return}const sym=card.dataset.symbol||card.querySelector('.tradeTitle strong')?.textContent?.trim()||'Trade',direction=card.querySelector('.direction')?.textContent?.trim()||'',resultEl=card.querySelector('.tradeResult'),main=resultEl?.childNodes?.[0]?.textContent?.trim()||resultEl?.textContent?.trim()||'',sub=resultEl?.querySelector('small')?.textContent?.trim()||'',time=card.querySelector('.tradeWhen')?.textContent?.trim()||'',isOpen=main==='OPEN';latest.innerHTML=`<div class="homeLatestHead"><span>Latest</span><button type="button" class="homeHistoryLink">View history</button></div><button type="button" class="homeLatestRow"><div><strong>${esc(sym)}${direction?` · ${esc(direction)}`:''}</strong><span>${esc(time)}</span></div><div class="homeLatestResult ${resultEl?.classList.contains('bad')?'bad':resultEl?.classList.contains('good')?'good':''}"><b>${isOpen?'Open':esc(main)}</b>${sub?`<span>${esc(sub)}</span>`:''}</div></button>`;latest.hidden=false;const openHistory=()=>{window.v2SetView?.('trades');setTimeout(()=>document.querySelector('[data-trade-mode="history"]')?.click(),0)};latest.querySelector('.homeHistoryLink')?.addEventListener('click',openHistory);latest.querySelector('.homeLatestRow')?.addEventListener('click',openHistory)}
